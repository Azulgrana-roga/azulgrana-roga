...
TELEFONO_ALBERGUE = "595981531063" # PONE ACA EL NUMERO DE BACILIO
...
def app():
    ...
    if pagina == "Lista de Usuarios":
        ...
        st.dataframe(pd.read_sql("SELECT * FROM mensajes ORDER BY id DESC LIMIT 20", conn), use_container_width=True)

    # === SOLO ESTA PESTAÑA FUE MODIFICADA - WHATSAPP WEB CON QR E HISTORIAL ===
    elif pagina == "WhatsApp Web":
        st.title("📱 WhatsApp Web - Centro de Mensajes")

        col_qr, col_env = st.columns([1,2])
        with col_qr:
            ...
            st.caption("1. Abrí WhatsApp Web\n2. En tu celular: WhatsApp > Dispositivos vinculados > Vincular\n3. Escaneá el QR de la PC")

        with col_env:
            st.subheader("Enviar WhatsApp al responsable")
            plantilla = pd.read_sql("SELECT mensaje FROM mensajes WHERE tipo='Predeterminado' LIMIT 1", conn).iloc[0]['mensaje']
            st.caption(f"Plantilla actual: {plantilla[:100]}...")
            tel = st.text_input("Número (ej: 0981123456)", key="tel_wp_nuevo")
            id_perm = st.number_input("ID Permiso (para agregar links de Aprobar/Rechazar)", min_value=0, step=1, key="id_wp_nuevo")
            mens = st.text_area("Mensaje", value=plantilla, height=120, key="mens_wp_nuevo")
            if st.button("Generar Link WhatsApp", type="primary", use_container_width=True, key="btn_wp_nuevo"):
                if not tel:
                    st.warning("Ingresá número")
                else:
                    txt = mens
                    if id_perm>0:
                        txt += f"\n\n✅ APROBAR: {LINK_BASE}/?accion=aprobar&id={id_perm}\n❌ RECHAZAR: {LINK_BASE}/?accion=rechazar&id={id_perm}"
                    url = f"https://wa.me/{limpiar_numero(tel)}?text={urllib.parse.quote(txt)}"
                    st.link_button("📲 Abrir WhatsApp", url, use_container_width=True)
                    c.execute("INSERT INTO mensajes (fecha, adolescente, contacto, tipo, mensaje, estado) VALUES (?,?,?,?,?,?)", (str(date.today()), f"ID {id_perm}", tel, "Enviado", txt, "Pendiente"))
                    conn.commit()

        st.markdown("---")
        st.subheader("Registrar respuesta que viene por WhatsApp")
        st.info("Cuando el responsable responde APROBADO o RECHAZADO por WhatsApp, pegá acá su respuesta para actualizar el Panel de Control")
        ca, cb = st.columns(2)
        with ca: id_r = st.number_input("ID Permiso", min_value=1, step=1, key="id_resp_final")
        with cb: cont_r = st.text_input("Contacto", key="cont_resp_final")
        resp = st.text_area("Pegá lo que respondió", key="resp_final", placeholder="Ej: Aprobado, puede salir")
        if st.button("✅ Procesar Respuesta y Actualizar Panel", type="primary", use_container_width=True, key="btn_proc_final"):
            if not resp:
                st.warning("Pegá la respuesta")
            else:
                txt_up = resp.upper()
                if "APROB" in txt_up: nuevo="Aprobado"
                elif "RECHAZ" in txt_up: nuevo="Rechazado"
                else:
                    st.error("No detecté APROBADO o RECHAZADO")
                    st.stop()
                c.execute("UPDATE permisos SET estado=? WHERE id=?", (nuevo, int(id_r)))
                c.execute("INSERT INTO mensajes (fecha, adolescente, contacto, tipo, mensaje, estado) VALUES (?,?,?,?,?,?)", (str(date.today()), f"ID {id_r}", cont_r, f"Respuesta WhatsApp {nuevo}", resp, nuevo))
                conn.commit()
                st.success(f"Permiso #{id_r} ahora {nuevo}")
                st.balloons()
                st.rerun()

        st.markdown("---")
        st.subheader("📜 Historial completo WhatsApp")
        st.dataframe(pd.read_sql("SELECT id, fecha, adolescente as Joven, contacto as Contacto, tipo as Tipo, estado as Estado, mensaje as Mensaje FROM mensajes ORDER BY id DESC", conn), use_container_width=True)
    ...
    elif pagina == "Portal Joven":
        st.title("Portal Joven - QR")
